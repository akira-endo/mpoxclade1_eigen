...
theta=(10/28)*((alpha/kappa)^(1/alpha));
...
for(t in 1:(1000*28)){
  x <- t*width
  aa[t] <- x*dweibull_lt(x,alpha,theta,10/28)
}
mu1 <- sum(aa)*width
...
for(t in 1:((upperl-1)/width)){
  x <- width*t 
  S[t] = width*dweibull_lt(x,alpha,theta,10/28)
}
wei_sum = sum(S)
...
Simulation_fn = function(n){
  ...
  for(t in 1:((upperl-1)/width)){
    x <- width*t 
    fx[t] = width*x*max(0,x-1)*dweibull_lt(x,alpha,theta,10/28)*exp(-(x*n)/(mu1*N))
    sx[t] = width*dweibull_lt(x,alpha,theta,10/28)*exp(-(x*n)/(mu1*N))
  }

...
theta=(7/28)*((alpha/kappa)^(1/alpha));
...
for(t in 1:(1000*28)){
  x <- t*width
  aa[t] <- x*dweibull_lt(x,alpha,theta,7/28)
}
mu1 <- sum(aa)*width
...
for(t in 1:((upperl-1)/width)){
  x <- width*t 
  S[t] = width*dweibull_lt(x,alpha,theta,7/28)
}
wei_sum = sum(S)
...
Simulation_fn = function(n){
  ...
  for(t in 1:((upperl-1)/width)){
    x <- width*t 
    fx[t] = width*x*max(0,x-1)*dweibull_lt(x,alpha,theta,7/28)*exp(-(x*n)/(mu1*N))
    sx[t] = width*dweibull_lt(x,alpha,theta,7/28)*exp(-(x*n)/(mu1*N))
  }
...
df <- mapply(Simulation_fn, seq(0,100*1500,100))
result_df <- df %>% t() %>% as.data.frame()
colnames(result_df) <- c("SAR","Infections","Reff_1")

write.csv(result_df, "outputs/outbreakpotential_msm/SAR_m_Reff_4w7d.csv")
